...
int main() {
    // porque es una variable de tipo texto
    string palabraSecreta = "programacion";
    // el numero de intentos permitidos al usuario
    int intentos = 5;
    // indicamos la longitud de la palbarasecreta con guiones
    string palabraAdivinada(palabraSecreta.length(), '_');
    string letrasAdivinadas;

    cout << "Bienvenido al Juego del Ahorcado!" << endl;
    //indicamos que si el usuario ingreso un dato en palabaraAdivinada y este cumple con con una letra de la pablarasecreta
    // una vez escrita la letra indicamos que le vamos a disminuir un intento aunque la palabra sea correcta.
    for (; intentos > 0 && palabraAdivinada != palabraSecreta; intentos--) {
        ...
        cin >> letra;
        //indicamo si la letra ingresada es ya fue ingreseda o el usuario la repite, le indicaremos que ya la ingreso.
        if (letrasAdivinadas.find(letra) != string::npos) {
            cout << "Ya adivinaste esa letra antes." << endl;
        } else {
            ...
            // si escribe una letra que no corresponde a la palabra le indicaremos que no es esa letra
            if (!letraAdivinada) {
                cout << "La letra no está en la palabra." << endl;
            }
        }
    }
    // si adivina la plabra se le indica al usario que la adivino
    if (palabraAdivinada == palabraSecreta) {
        cout << "¡Felicidades, adivinaste la palabra! La palabra era: " << palabraSecreta << endl;
    } else {
        // si no adivina en la cantidad de intentos disponible le indicaremos que agoto sus intentos
        cout << "¡Oh no, has agotado tus intentos! La palabra era: " << palabraSecreta << endl;
    }
...
# Número de intentos permitidos al jugador
intentos = 5
# indicamos la longitud de la palbarasecreta con guiones
palabraAdivinada = "_" * len(palabraSecreta)
letrasAdivinadas = ""

print("Bienvenido al Juego del Ahorcado!")
# indicamos que si el usuario ingreso un dato en palabaraAdivinada y este cumple con con una letra de la pablarasecreta
# una vez escrita la letra indicamos que le vamos a disminuir un intento aunque la palabra sea correcta.
while intentos > 0 and palabraAdivinada != palabraSecreta:
    print(f"Palabra a adivinar: {palabraAdivinada}")
    print(f"Intentos restantes: {intentos}")

    letra = input("Ingresa una letra: ").lower()  # Convertir a minúsculas para evitar problemas de mayúsculas y minúsculas
#indicamos si la letra ingresada es ya fue ingreseda o el usuario la repite, le indicaremos que ya la ingreso.
...
if palabraAdivinada == palabraSecreta:
    print(f"¡Felicidades, adivinaste la palabra! La palabra era: {palabraSecreta}")
else:
    print(f"¡Oh no, has agotado tus intentos! La palabra era: {palabraSecreta}")
```